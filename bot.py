    ...
    if context.args:
        ...
        if ref_id != user_id:
            ...
            if ref_id in data and user_id not in data[ref_id]["referrals"]:
                ...
                save_json(DATA_FILE, data)
                await context.bot.send_message(chat_id=ref_id, text=f"🎉 Someone joined with your referral link!")
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rank = "
".join([f"👤{i+1}. {uid}: {len(info['referrals'])} invites" for i, (uid, info) in enumerate(leaderboard[:5])])
...
async def handle_admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        if uid in withdrawals:
            ...
            save_json(WITHDRAWALS_FILE, withdrawals)
            await query.message.reply_text(f"✅ Approved withdrawal for {uid}")
        ...
        if uid in withdrawals:
            ...
            save_json(WITHDRAWALS_FILE, withdrawals)
            await query.message.reply_text(f"🚫 Declined withdrawal for {uid}")
